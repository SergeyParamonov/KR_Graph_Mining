...
To set up a nested solver for a formula $\exists_{SO} X : \phi$, we must establish a vocabulary $V$ and a theory $T$ over $V$ for this solver. 
To this end, we first identify the symbols used in $\phi$.
These symbols, together with the symbol X from the quantification itself, are collected in the new vocabulary $V$.
Next, it is possible to use the formula $\phi$ as the theory $T$.
