...
Quantification over second order objects uses annotated quantifiers ($\exists_{SO}$ and $\forall_{SO}$) and must be typed \matthias{(any unary predicate represents a type)}, e.g. line~\ref{lstline:exists}.
...
We now propose a way in which a solver can provide this additional support\matthias{, and potentially even improve performance}.
...
First, all second order universal quantifications $\forall_{SO} X : \phi$ are rewritten to existential quantifications $\exists_{SO} X : \neg \phi$ or $\neg \exists_{SO} X : \phi$.
...
Because our \NP-solver conforms to this interface, we can modify the \NP-solver such that it calls a new instance of itself as an oracle to evaluate the truth of these formulas.
As it is possible to nest these solvers arbitrarily deep, we can now solve a formula of the form $\exists_{SO} X : \phi$
