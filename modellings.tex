...
In this section, we show how state-of-the-art KR systems without support for higher order logic, such as IDP and ASP, can model the graph mining problem and its higher order features using encoding techniques.
...
in \textbf{Listing}~\ref{lst:multiglobal}.
% and visualised as the dashed shapes in \textbf{Fig.}~\ref{fig:LocalCoherence}.
...
\begin{minipage}{0.80\linewidth}
  \begin{lstlisting}[style=small,mathescape,caption=Higher order predicate modeling the set $\graphset{G}$ of \textbf{Def.}~\ref{def:gm2}.,label=lst:HOPred]
...
This global edge relation now corresponds to the \emph{disjoint} or \emph{tagged union} of the graphs' edge relations, where the tags are drawn from a set $G$ consisting of graph identifiers.
