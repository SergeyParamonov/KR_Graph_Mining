...
\sergey{to do: CP(graph) \citep{cp_graph}, reference to Siegfried survey \citep{subtree_overview} -- many ways to define matching ``subtree'' for example, Anton's + Sigfried's mining under theta subsumption \citep{theta_subsumption} -- means we need different matching}
...
\paragraph{Issue 1}
The restriction to \emph{Existential} Second Order means we cannot express the set of example graphs, as specified in Definition~\ref{def:gm2}, using a higher-order predicate (shown in Listing~\ref{lst:HOPred}).
...
This way to represent the information about graphs is shown in Listing~\ref{lst:multiglobal}.
...
A more workable solution is to represent each characteristic property, such as the edge relation, by a single general relation for all graphs, as shown in Listing~\ref{lst:indexedglobal}.
This entity behaves the way it should for a specific graph instance based on an additional argument serving as an identifier for the graph of interest.
...
\paragraph{Issue 2}
Having solved\matthias{circumvented?} our first issue, we would like to express the homomorphic property, as shown in Listing \ref{QuantifyOutsideVocabulary}.
