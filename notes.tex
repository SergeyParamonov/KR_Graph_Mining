...
Frequent Pattern Mining, one of the ''super-problems`` of Data Mining, has received significant attention over the years \citep{pattern_mining_book}. Especially useful in applications  turned out to be structured mining such as frequent graph mining, where one is looking for sub-structures that often occur in the data, e.g., a graph that is a subgraph of many graphs in the dataset. Recently, there is a line of work towards declarative pattern mining \citep{tias_declarative_pattern_mining} and today look into declarative structured pattern mining that just started receiving attention in both data mining and knowledge representation communities \citep{cp_sequence_mining,ilp_graph_mining}. 
