...
\begin{definition}
A graph $G = (V,E,L)$ is \emph{connected} iff for each pair of vertices $V$ and $V'$, there exists an edge $\langle V,V'\rangle \in E$ or there exists a sequence $V \ldots V_{1} \ldots V_{n} \ldots V'$ such that there exist edges $\langle V,V_{1} \rangle$, $\langle V_{i},V_{i+1}\rangle$ and $\langle V_{n},V'\rangle \in E$.
\end{definition}


\begin{definition}
A graph homomorphism $f$ between two labeled graphs $G = (V,E,L)$ and $G' = (V',E',L')$, where $V$ is the set of vertices, $E$ the set of edges and $L$ the labeling function, is a mapping $V \rightarrow V'$ from vertices of $G$ to vertices of $G'$ s.t. 
\begin{itemize}
\item $\forall u,v \in V, \langle u,v\rangle \in E \implies \langle f(u),f(v)\rangle \in E'$ (the mapping preserves edges), and 
...
A graph isomorphism $f$ from labeled graph $G = (V,E,L)$ to $G' = (V',E',L')$ is a \emph{one-to-one} mapping $V \rightarrow V'$ 
...
Let $\mathcal{G}$ be a set of graphs, closed under isomorphism.
...
\label{def:gm2}
Given a set of vertices $V$ and a set $G$ of $\langle E, L, C\rangle$ triples,
...
we look for a (set $P$ of)\todo{cleanly extend this definition to multiple graphs, or separate the single and multiple pattern case again} graph(s) represented by tuple(s) $\langle E_{p}, L_{p}\rangle$ such that
for at least $N_{+}$ of the triples $\langle E, L, C\rangle$ with $C=Pos$, and for at most $N_{-}$ of such triples with $C=Neg$, there exists a function $f$ s.t. $\forall u,v \in V, \langle u,v\rangle \in E_{p} \implies \langle f(u),f(v)\rangle \in E$ and $\forall v \in V : L_{p}(v) = L(f(v))$.

\todo{draft: possible different wording}
\begin{itemize}
\item $\#\Big\lbrace \langle E,L,Pos \rangle \; | \; \langle E,L,Pos \rangle \in G \; \wedge \; \big(\exists f : \forall v \in V : L_{p}(v) = L(f(v)) \; \wedge \; \forall x,y \in V : E_{p}(x,y) \implies E(f(x),f(y))\big)\Big\rbrace \geq N_{+}$

\item $\#\Big\lbrace \langle E,L,Neg \rangle \; | \; \langle E,L,Neg \rangle \in G \; \wedge \; \big(\exists f : \forall v \in V : L_{p}(v) = L(f(v)) \; \wedge \; \forall x,y \in V : E_{p}(x,y) \implies E(f(x),f(y))\big)\Big\rbrace \leq N_{-}$
\end{itemize}
\end{definition}
