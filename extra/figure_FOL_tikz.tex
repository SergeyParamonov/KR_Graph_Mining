...
\pgfsetlayers{background,main}
 ...
 \begin{figure}[thb]
    \caption{A visualization for the first order query mining model}
    \label{fig:fol}
    \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,swap,node distance=1.5cm,
      ...
      \node (f) [right of =tcenter, xshift=1.3cm] {\Large{$\exists f(g,x)$}};
      \draw (tcenter) -- (f);

      \draw [->] (f) to [bend left=45]  node [midway, above left] {$f(g_1,x) \mapsto y$} (g1center);
      \draw [->] (f) to [bend left=65]  node [midway, above right] {$f(g_2,x) \mapsto y$} (g2center);
      \draw [->] (f) to [bend left=25]  node [midway, below left] {$f(g_3,x) \mapsto y$} (g3center);
