...
%Declarative programming distinguishes itself by the ease with which 
...
%On the other hand, ProB~\citep{journals/sttt/LeuschelB08}, as a representative of formal specification languages, extends predicate logic with set theory and offers higher order datastructures.
...
%can express inductive definitions using either its completion or using a built-in transitive closure operator, this comes with a significant loss of readability.
...
A first class of \emph{pattern mining} is \emph{unstructured mining}, such as \emph{itemset mining}, where the pattern is a set of items without any additional structural relation between the different items. 
%Because it does not require any structure, the \emph{unstructured mining} task does not compose multiple smaller problems.
%Therefore the rise in computational complexity associated with problem composition does not occur and 
...
%, or the order of elements in a sequence.
...
%required by the graph mining problem
%introduces subproblems:
% within the graph mining problem: 
%We need to specify when a graph occurs within other graphs, leading to the \NP-complete problem of graph homomorphism~\citep{Lev73}, 
