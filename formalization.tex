...
This pattern $\graph{P}$ must be \emph{homomorphic} with at least $N_{+}$ positive, while being homomorphic with at most $N_{-}$ negative examples. We call these homomorpishms the positive (negative) homomorphisms, and the restriction on their number the positive (negative) homomorphic property, respectively.
\end{definition}
...
Take, for example, the problem set shown in Figure~\ref{fig:ex1}. We assume all nodes have the same label.
There is one positive example (Fig~\ref{fig:pos}), and one negative example (Fig~\ref{fig:neg}). 
Figure \ref{fig:templ} shows the template graph.
...
As the domains of this predicate range over predicates and functions, this predicate is a higher order predicate.
